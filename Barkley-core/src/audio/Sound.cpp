...
namespace barkley{ namespace audio {
		...
		: m_Name(name), m_Filename(filename), m_Playing(false), m_Count(0)
	{
		...
		if (split.size() < 2)
		{
			...
			BARKLEY_ERROR("[Sound] invalid file name '", m_Filename.c_str(), "'!");
			return;
		}
...
// 		);
			...
			if (m_Sound == nullptr)
			{
				...
				BARKLEY_ERROR("[Sound] Could not load file '", m_Filename.c_str(), "'!");
			}
#endif	
	}
	...
	void Sound::SetGain(float gain)
	{
		if (!m_Playing)
		{
			BARKLEY_WARN("[Sound] Cannot set gain! Sound is not currently playing!");
			return;
		}
		m_Gain = gain;
#ifdef BARKLEY_PLATFORM_WEB

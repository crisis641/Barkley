...
namespace barkley { namespace graphics{
	...
	Sprite::Sprite(Texture* texture)
		: Renderable2D(maths::vec3(0, 0, 0), maths::vec2((float)texture->GetWidth(), (float)texture->GetHeight()), 0xffffffff), position(m_Position), size(m_Size)
	{
		m_Texture = texture;
	}

	Sprite::Sprite(float x, float y, Texture* texture)
		: Renderable2D(maths::vec3(x, y, 0), maths::vec2((float)texture->GetWidth(), (float)texture->GetHeight()), 0xffffffff), position(m_Position), size(m_Size)
	{
		m_Texture = texture;
	}

...
namespace barkley { namespace graphics {
	...
	Shader* Shader::FromSrc(const char* name, const char* vertSrc, const char* fragSrc)
	{
	...
	GLuint Shader::Load(const char* vertSrc, const char* fragSrc)
	{
		...
		if (result == GL_FALSE)
		{
			...
			BARKLEY_ERROR("[Shader] Failed to compile vertex shader");
			BARKLEY_ERROR(&error[0]);
			BARKLEY_ASSERT(false, "[Shader] Failed to compile vertex shader");
		...
		if (result == GL_FALSE)
		{
			...
			BARKLEY_ERROR("[Shader] Failed to compile fragment shader");
			BARKLEY_ERROR(&error[0]);
			BARKLEY_ASSERT(false, "[Shader] Failed to compile fragment shader");
